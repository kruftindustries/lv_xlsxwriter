...
/* ============================================================================
 * Sparkline Enumeration Types
 * ============================================================================ */

typedef enum lxw_sparkline_type {
    LXW_SPARKLINE_LINE = 0,
    LXW_SPARKLINE_COLUMN = 1,
    LXW_SPARKLINE_WIN_LOSS = 2
} lxw_sparkline_type;

typedef enum lxw_sparkline_empty_cells {
    LXW_SPARKLINE_EMPTY_CELLS_GAP = 0,
    LXW_SPARKLINE_EMPTY_CELLS_ZERO = 1,
    LXW_SPARKLINE_EMPTY_CELLS_SPAN = 2
} lxw_sparkline_empty_cells;

typedef enum lxw_sparkline_axis_type {
    LXW_SPARKLINE_AXIS_AUTOMATIC = 0,
    LXW_SPARKLINE_AXIS_GROUP = 1,
    LXW_SPARKLINE_AXIS_CUSTOM = 2
} lxw_sparkline_axis_type;

typedef struct lxw_sparkline_options {
    unsigned long range;           /* const char* - Required: "Sheet1!A1:E1" */
    unsigned long location;        /* const char* - Optional: cell location string */
    uint8_t type;                  /* lxw_sparkline_type */
    uint8_t style;                 /* 0-35 predefined styles */
    uint8_t high_point;            /* Show high point marker */
    uint8_t low_point;             /* Show low point marker */
    uint8_t negative_points;       /* Show negative point markers */
    uint8_t first_point;           /* Show first point marker */
    uint8_t last_point;            /* Show last point marker */
    uint8_t markers;               /* Show all markers (line only) */
    lxw_color_t series_color;      /* RGB color for series (0 = use style) */
    lxw_color_t negative_color;    /* RGB color for negative values */
    lxw_color_t markers_color;     /* RGB color for markers */
    lxw_color_t first_color;       /* RGB color for first point */
    lxw_color_t last_color;        /* RGB color for last point */
    lxw_color_t high_color;        /* RGB color for high point */
    lxw_color_t low_color;         /* RGB color for low point */
    double min;                    /* Minimum axis value */
    double max;                    /* Maximum axis value */
    uint8_t min_axis_type;         /* lxw_sparkline_axis_type */
    uint8_t max_axis_type;         /* lxw_sparkline_axis_type */
    uint8_t show_axis;             /* Show horizontal axis */
    uint8_t reverse;               /* Reverse data order (right to left) */
    uint8_t show_hidden;           /* Show data in hidden rows */
    uint8_t empty_cells;           /* lxw_sparkline_empty_cells */
    double weight;                 /* Line weight (default 0.75) */
    unsigned long date_axis;       /* const char* - Date axis range */
} lxw_sparkline_options;

/* ============================================================================
 * Chart Axis Time Unit Enumeration
 * ============================================================================ */

typedef enum lxw_chart_axis_time_unit {
    LXW_CHART_AXIS_TIME_UNIT_DAYS = 0,
    LXW_CHART_AXIS_TIME_UNIT_MONTHS = 1,
    LXW_CHART_AXIS_TIME_UNIT_YEARS = 2
...
lxw_error worksheet_add_sparkline(lxw_worksheet worksheet, lxw_row_t row, lxw_col_t col, lxw_sparkline_options *options);
